...
def add_payment_method():
    ...
    if form.validate_on_submit():
        ...
        import json
        try:
            if form.details.data and form.details.data.strip():
                method.details = json.loads(form.details.data.strip())
                # Логируем успешный парсинг
                print(f"✅ JSON успешно парсится: {method.details}")
            else:
                method.details = None
        except json.JSONDecodeError as e:
            # Конкретная ошибка JSON
            flash(f'Ошибка в JSON формате: {str(e)}. Проверьте кавычки - используйте только двойные!', 'error')
            methods = PaymentMethod.query.order_by(PaymentMethod.order).all()
            return render_template('admin/payment_methods.html', form=form, methods=methods)
        except Exception as e:
            # Другие ошибки
            flash(f'Ошибка обработки данных: {str(e)}', 'error')
            methods = PaymentMethod.query.order_by(PaymentMethod.order).all()
            return render_template('admin/payment_methods.html', form=form, methods=methods)
